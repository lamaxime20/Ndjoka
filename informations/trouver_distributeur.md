> **Mise à jour post-refonte (voir `REFONTE_NDJOKA.md`, section 10)** : la section 6 « Produits en développement » ci-dessous ne listait que Chin Chin Ndjoka et Jus d'oseille Ndjoka. La liste confirmée compte désormais 5 produits : Chips de patate, Caramels, Jus d'oseille, Biscuits, Chin chin. Voir `vue/src/services/catalogueProduits.js` (`PRODUITS_EN_DEVELOPPEMENT`) pour la donnée réellement utilisée par le site.

# PAGE TROUVER UN DISTRIBUTEUR

## SECTION 1 — HERO

### Titre

Un réseau qui rapproche Ndjoka des consommateurs partout où ils se trouvent

### Texte

Déjà présent dans plusieurs villes et pays, le réseau Ndjoka continue de grandir pour rendre ses produits accessibles à toujours plus de consommateurs.

### CTA principal

Rejoindre le réseau Ndjoka

### CTA secondaire

Trouver un distributeur

### Image

Grande photo immersive.

Privilégier :

* plusieurs cartons de produits Ndjoka
* présentoirs Ndjoka
* produits visibles
* environnement réel de distribution

L'image doit faire penser :

"Cette marque est déjà présente sur le terrain."

---

## SECTION 2 — LA FORCE DU RÉSEAU

### Badge

Réseau en expansion

### Titre

Une présence qui grandit chaque année

### Texte

Ndjoka développe progressivement son réseau de distribution afin de rapprocher ses produits des consommateurs et de créer de nouvelles opportunités commerciales.

### Chiffres

15+

Villes couvertes

10+

Pays représentés

Plusieurs

Concessionnaires et partenaires

### Image

Aucune.

Cette section doit respirer.

Les chiffres sont les héros.

---

## SECTION 3 — CARTE D'EXPANSION

### Badge

Présence internationale

### Titre

Une marque présente au Cameroun et au-delà

### Texte

Chaque nouveau point de distribution renforce la proximité entre Ndjoka et ses consommateurs.

Explorez les pays où le réseau est déjà représenté.

### Carte interactive

Au survol d'un pays :

Afficher :

Nom du concessionnaire

Téléphone

Ville

Localisation courte

Là, on doit designer une carte du monde avec tous les différents concessionnaires et je vais donner le document PDF de la notice pour avoir ces informations

---

## SECTION 4 — TROUVER UN DISTRIBUTEUR

### Badge

Points de distribution

### Titre

Trouvez un distributeur proche de chez vous

### Texte

Recherchez une ville ou un pays pour consulter les points de distribution disponibles et leurs coordonnées.

### Champ de recherche

Ville

Pays

### Résultat

Téléphone

Localisation détaillée

### Image

Aucune.

Le moteur de recherche est la fonctionnalité principale.

---

## SECTION 5 — POURQUOI REJOINDRE LE RÉSEAU NDJOKA ?

### Badge

Opportunité

### Titre

Pourquoi des entrepreneurs rejoignent Ndjoka

### Texte

Rejoindre Ndjoka, c'est participer au développement d'une marque africaine ambitieuse tout en construisant une activité locale durable.

### Avantage 1

Une marque en croissance

Profitez de la visibilité d'une marque qui continue d'étendre sa présence.

### Avantage 2

Des produits appréciés

Distribuez des produits déjà présents dans plusieurs villes et marchés.

### Avantage 3

Un réseau structuré

Évoluez aux côtés d'autres concessionnaires et partenaires.

### Avantage 4

Une opportunité de développement

Grandissez avec une marque qui prépare déjà ses prochaines étapes.

### Image

Photo authentique.

Entrepreneur africain.

Produits Ndjoka visibles.

Sourire naturel.

Aucune pose corporate.

Le visiteur doit pouvoir se projeter.

---

## SECTION 6 — LA VISION NDJOKA

### Badge

Notre vision

### Titre

Construire l'une des références agroalimentaires africaines de demain

### Points clés

#### Valoriser les produits africains

Ndjoka poursuit une ambition claire : valoriser les produits africains à travers une distribution moderne, accessible et durable.

#### Une présence déjà établie

Aujourd'hui, la marque est déjà présente dans plusieurs villes et pays grâce à son réseau de distributeurs, concessionnaires et partenaires.

#### Une croissance en marche

Demain, cette dynamique se poursuivra avec l'élargissement de la gamme de produits, le renforcement du réseau de distribution et l'ouverture de nouvelles opportunités commerciales à travers l'Afrique et au-delà.

### Sous-section

Les prochaines étapes de l'univers Ndjoka

### Texte

L'innovation fait partie de la croissance de Ndjoka. Plusieurs produits sont actuellement en développement afin d'accompagner l'expansion de la marque.

### Produits en développement

Chin Chin Ndjoka

Prochainement

Jus d'oseille Ndjoka

Prochainement

---

## SECTION 7 — CTA FINAL

### Titre

Rejoignez une marque qui continue de grandir

### Texte

Chaque nouveau concessionnaire contribue à rapprocher les produits Ndjoka des consommateurs et participe à la construction d'un réseau africain en pleine expansion.

### CTA principal

Devenir concessionnaire Ndjoka

### CTA secondaire

Contacter l'équipe

\

L'utilisateur doit terminer la page en pensant :

"Cette marque est déjà présente."

Puis :

"Cette marque grandit."

Puis :

"Je pourrais faire partie de cette croissance."
